...
[ // <--
  ...
  function stream(req, res, options, _, server, clb) {
    ...
    // Enable developers to grab chunked data from the stream rather than one blob at the end.
    // Respect why it was chunked in the first place.
    proxyReq.on('data', function(chunk) {
      if(server) { server.emit('proxyChunk', chunk, req, res, options); }
    });
